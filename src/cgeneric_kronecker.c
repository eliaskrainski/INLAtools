...
//   abort();
	    ...
	    *(void **)(&d12cache->model2_func) =
		...
		if (hModule) {
		    *(void **)(&d12cache->model1_func) = (inla_cgeneric_func_tp *) ((void *) GetProcAddress(hModule, &d12cache->dataM1->chars[0]->chars[0]));
		}
	    }
	    if(!(&d12cache->model2_func)) {
		HMODULE hModule = GetModuleHandle(NULL);
		if (hModule) {
		    *(void **)(&d12cache->model2_func) = (inla_cgeneric_func_tp *) ((void *) GetProcAddress(hModule, &d12cache->dataM2->chars[0]->chars[0]));
		}
	    }
#endif
	    const char *error = NULL;
	    if (!d12cache->model1_func && (error = lt_dlerror()) != NULL) {
		fprintf(stderr, "%s\n", error);
		exit(1);
	    }
	    if (!d12cache->model2_func && (error = lt_dlerror()) != NULL) {
		fprintf(stderr, "%s\n", error);
		exit(1);
	    }
#else
	    *(void **)(&d12cache->model1_func) =
		dlsym(d12cache->handle1, &d12cache->dataM1->chars[0]->chars[0]);
	    *(void **)(&d12cache->model2_func) =
		dlsym(d12cache->handle2, &d12cache->dataM2->chars[0]->chars[0]);
#endif
    ...
    {
	// return c(LOG_PRIOR)
	ret1 = d12cache->model1_func(INLA_CGENERIC_LOG_PRIOR,
				     &theta[0], d12cache->dataM1);
