...
(r <- 0.9)
R3 <- inla.as.sparse(sparseMatrix(
    i = c(1:3, 1:2, 2:3),
    j = c(1:3, 2:3, 1:2),
...
all.equal(inla.as.sparse(R321),
          inla.as.sparse(Q1))

### using Kronecker between cgeneric models for each dim
...
## kronecker
cg21 <- kronecker(cg2, cg1, useINLAprecomp = FALSE)
cg321 <- kronecker(cg3, cg21, useINLAprecomp = FALSE)

## test1
cg21Q <- prec(cg21, theta = 0)
all.equal(inla.as.sparse(R21),
          inla.as.sparse(cg21Q))

## test2
cg321Q <- prec(cg321, theta = 0)

all.equal(inla.as.sparse(R321),
          inla.as.sparse(cg321Q))
